...
if (argv.help || hasInvalidOptions(argv) || validCommands.indexOf(argv._[0]) < 0) {
    console.log("Usage:");
    console.log(" * api-dashboard build");
    console.log(" * api-dashboard start [-p 8080] [-a user=password]");
    process.exit();
}
...
if (argv._[0] === 'start') {
    ...
    if (port) {
        process.env['api-dashboard-port'] = port;
    }
    if (auth) {
        auth = auth.split('=');
        process.env['api-dashboard-name'] = auth[0];
        process.env['api-dashboard-pass'] = auth[1];
    }
