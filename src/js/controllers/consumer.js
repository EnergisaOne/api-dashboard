angular.module('app').controller("ConsumerController", ["$scope", "Kong", "$location", "$routeParams", "consumer", "Alert", "$route", function ($scope, Kong, $location, $routeParams, consumer, Alert, $route) {
    if ($routeParams.id) {
        ...
        $scope.title = "Editar Requisitante";
        ...
        $scope.title = "Adicionar Requisitante";
    ...
    $scope.save = function () {
        Kong.put('/consumers', $scope.consumer).then(function () {
            if ($routeParams.id) {
                Alert.success('Requisitante atualizado');
            } else {
                Alert.success('Requisitante adicionado');
                // clearing inputs.
                $scope.consumer = {};
            }
            // clearing errors.
